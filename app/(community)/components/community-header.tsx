...
export function CommunityHeader({
    ...
    const [currentCommunityData, setCurrentCommunityData] =
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                console.log(
                    "Fetching header data for community:",
                    currentCommunity,
                );

                const [userCommunitiesRes, currentCommunityRes] =
                    await Promise.all([
                        isAuthenticated
                            ? communitiesApi.getMyJoined().catch((err) => {
                                  console.error(
                                      "Error fetching user communities:",
                                      err,
                                  );
                                  return null;
                              })
                        ...
                        communitiesApi
                            .getBySlug(currentCommunity)
                            .catch((err) => {
                                console.error(
                                    "Error fetching current community:",
                                    err,
                                );
                                return null;
                            }),
    ...
    const handleSearch = (e: React.FormEvent) => {
        ...
        if (searchQuery.trim()) {
            router.push(
                `/${creatorSlug}/${currentCommunity}/home?search=${encodeURIComponent(searchQuery.trim())}`,
            );
        }
    };
    ...
    const mobilePrimaryNavigationItems = useMemo(
        () =>
            headerNavigationItems.filter((item) =>
                ["/home", "/courses", "/challenges", "/sessions"].includes(
                    item.href,
                ),
            ),
    ...
    const mobilePrimaryHrefs = new Set(
        mobilePrimaryNavigationItems.map((item) => item.href),
    );
    ...
    const isMoreActive = headerNavigationItems.some(
        (item) =>
            !mobilePrimaryHrefs.has(item.href) && isRouteActive(item.href),
    );
    if (loading) {
        return (
            ...
            />
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        <NotificationsBell
                            ...
                            communitySlug={currentCommunity}
                        />
                        ...
                        <DropdownMenu>
                            <DropdownMenuTrigger asChild>
                                <Link href="/profile">
                                    <Button
                                        variant="ghost"
                                        className="hidden sm:flex items-center space-x-2 px-3 rounded-full"
                                    >
                            ...
                            return (
                                <Link
                                    ...
                                    className={cn(
                                        ...
                                        isActive
                                            ? "bg-gradient-to-b from-primary-50 to-primary-100/70 text-primary-700 shadow-[inset_0_1px_0_rgba(255,255,255,0.5)]"
