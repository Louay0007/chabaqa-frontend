...
export default function ChapterTabs({
    ...
    const handleGoToNextChapterClick = async () => {
        ...
        if (!nextChapterId || !onGoToNextChapter) {
            console.warn(
                "[CourseNextFlow] ChapterTabs next navigation blocked",
                {
                    reason: !nextChapterId
                        ? "No next chapter available"
                        : "Missing next chapter handler",
                    currentChapterId,
                },
            );
            return;
        ...
        try {
            ...
            console.info(
                "[CourseNextFlow] ChapterTabs next navigation requested",
                {
                    currentChapterId,
            ...
            console.error(
                "[CourseNextFlow] ChapterTabs next navigation failed",
                {
                    currentChapterId,
                    nextChapterId: String(nextChapterId),
                    error,
    ...
    return (
        <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-6 h-auto p-1.5 bg-muted/50">
                <TabsTrigger
                    value="content"
                    className="py-2.5 md:py-3 text-xs md:text-sm font-medium data-[state=active]:bg-background data-[state=active]:shadow-sm"
                >
                    Content
                </TabsTrigger>
                <TabsTrigger
                    value="geo"
                    className="py-2.5 md:py-3 text-xs md:text-sm font-medium gap-1 data-[state=active]:bg-background data-[state=active]:shadow-sm"
                >
                ...
                <TabsTrigger
                    ...
                    className="py-2.5 md:py-3 text-xs md:text-sm font-medium gap-1 data-[state=active]:bg-background data-[state=active]:shadow-sm"
                >
                    <Captions className="h-3 w-3 md:h-4 md:w-4 text-violet-500" />
                    ...
                    <CardContent className="space-y-4">
                        {chapterContent ? (
                            ...
                            <p className="text-muted-foreground italic text-sm md:text-base">
                                No content description for this chapter.
                            </p>
                        )}
                        ...
                        <div className="flex flex-wrap gap-3 mt-6 pt-4 border-t">
                            ...
                            onGoToNextChapter ? (
                                <Button
                                    type="button"
                                    onClick={() =>
                                        void handleGoToNextChapterClick()
                                    }
            ...
            <TabsContent value="geo" className="mt-4 md:mt-6">
                {courseId && currentChapter?.id ? (
                    <GeoChat
                        ...
                        chapterName={
                            currentChapter?.title || currentChapter?.titre
                        }
                    />
                ) : (
                    <Card className="border shadow-sm">
                        <CardContent className="py-12 text-center text-muted-foreground">
                            ...
                            <p className="text-sm md:text-base">
                                Please select a chapter to use Geo.
                            </p>
                        </CardContent>
                    </Card>
                )}
                ...
                {courseId && currentChapter?.id ? (
                    ...
                    <Card className="border shadow-sm">
                        <CardContent className="py-12 text-center text-muted-foreground">
                            ...
                            <p className="text-sm md:text-base">
                                Please select a chapter to take a quiz.
                            </p>
                        </CardContent>
                    </Card>
                )}
            ...
            <TabsContent value="notes" className="mt-4 md:mt-6">
                <Card className="border shadow-sm">
                    <CardHeader className="pb-4">
                        ...
                        <CardDescription className="text-xs md:text-sm">
                            Additional notes and tips from the instructor
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {chapterNotes ? (
                            ...
                            <div className="text-center py-12 text-muted-foreground">
                                ...
                                <p className="text-sm md:text-base">
                                    No instructor notes for this chapter
                                </p>
                            </div>
                        )}
                    </CardContent>
                            ...
                            chapterResources.map(
                                (resource: any, index: number) => (
                                    <div
                                        ...
                                        className="flex items-center justify-between p-3 md:p-4 border-2 rounded-lg hover:bg-gray-50 hover:border-gray-300 transition-all"
                                    >
            ...
            <TabsContent value="transcript" className="mt-4 md:mt-6">
                {courseId && (chapterId || currentChapter?.id) ? (
                    <TranscriptPanel
                        chapterId={
                            chapterId ?? String(currentChapter?.id ?? "")
                        }
                    ...
                    <Card className="border shadow-sm">
                        <CardContent className="py-12 text-center text-muted-foreground">
                            <Captions className="h-12 w-12 md:h-16 md:w-16 mx-auto mb-4 opacity-50" />
                            <p className="text-sm md:text-base">
                                Please select a chapter to view transcript.
                            </p>
                        </CardContent>
                    </Card>
                )}
