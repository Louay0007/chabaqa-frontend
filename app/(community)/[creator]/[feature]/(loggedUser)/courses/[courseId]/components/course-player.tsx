...
export default function CoursePlayer({
    ...
    >({});
    const [chapterAccessReason, setChapterAccessReason] = useState<
        Record<string, string | undefined>
    >({});
    const accessCheckInFlight = useRef<
    ...
    const [watchTimeOverride, setWatchTimeOverride] = useState<number | null>(
        null,
    );
    const [videoDurationOverride, setVideoDurationOverride] = useState<
        number | null
    >(null);
    const lastProgressRefreshRef = useRef<number>(0);
    const progressBackoffRef = useRef<{
        attempts: number;
        nextAllowed: number;
    }>({
        attempts: 0,
    ...
    const trackingSentRef = useRef<{ start: boolean; complete: boolean }>({
        start: false,
        complete: false,
    });
    ...
    const effectiveUnlockMessage =
        unlockMessage ||
        (typeof course?.unlockMessage === "string"
            ? course.unlockMessage
            : undefined);
    const previewLockedReason =
        "Only the first chapter is available as preview. Enroll to unlock full course.";
    const mapLockCodeToReason = useCallback(
        (lockCode?: string): string | undefined => {
            switch (lockCode) {
    ...
    const unlockedMap = useMemo(() => {
        ...
        return new Map<string, any>(
            unlockedChapters.map((c: any) => [String(c.id), c]),
        );
    }, [unlockedChapters]);
    ...
    const resolveSequentialAccessLocally = useCallback(
        (chapterId: string): { canAccess: boolean; reason?: string } => {
            ...
            const chapterIndex = allChapters.findIndex(
                (c: any) => String(c.id) === resolvedChapterId,
            );
            if (chapterIndex === -1) {
            ...
            const previousProgress = Array.isArray(enrollment?.progress)
                ? enrollment.progress.find(
                      (p: any) =>
                          String(p.chapterId) === String(previousChapter?.id),
                  )
                : null;
    ...
    useEffect(() => {
        const onChapterComplete = async () => {
            if (courseSession) {
                const currentId = currentChapterRef.current?.id
                    ? String(currentChapterRef.current.id)
                    : null;
                if (currentId) {
    ...
    const resolveChapterAccess = useCallback(
        async (
            ...
            const requiresPaidAccess =
                Boolean(chapter.isPaidChapter) && !Boolean(chapter.isPreview);
            if (requiresPaidAccess) {
                // Ask backend for paid access decision (covers freemium membership rule)
                try {
                    const paidAccessRaw =
                        await coursesApi.checkChapterAccessPaid(
                            resolvedCourseId,
                            resolvedChapterId,
                        );
                    const paidAccess =
                        (paidAccessRaw as any)?.data || paidAccessRaw;
                    const paidAllowed = Boolean((paidAccess as any)?.canAccess);
                    if (!paidAllowed) {
                        const reasonFromCode = mapLockCodeToReason(
                            (paidAccess as any)?.lockCode,
                        );
                        return {
                            canAccess: false,
                            reason:
                    ...
                    if (!isUserEnrolled) {
                        return {
                            canAccess: false,
                            reason:
                                effectiveUnlockMessage || "Enrollment required",
                        };
                    }
                }
            }
    ...
    const ensureChapterAccessCached = useCallback(
        async (chapterId: string): Promise<boolean> => {
            ...
            if (
                !effectiveSequentialProgressionEnabled &&
                typeof accessibleChapters[key] === "boolean"
            ) {
    ...
    const isChapterAccessible = useCallback(
        (chapterId: string) => {
            ...
            if (!isUserEnrolled) {
                return (
                    Boolean(firstChapterId && key === firstChapterId) ||
            ...
            return (
                Boolean(chapter.isPreview) || !Boolean(chapter.isPaidChapter)
            );
        },
        [
            allChapters,
    ...
    useEffect(() => {
        ...
        if (!Array.isArray(unlockedChapters) || unlockedChapters.length === 0)
            return;
    ...
    const tryAutoAdvanceToNext = useCallback(
        async (
            ...
            const canAccessNext =
                await ensureChapterAccessCached(nextChapterId);
    ...
    useEffect(() => {
        ...
        const targetChapter = allChapters.find(
            (c: any) => String(c.id) === String(pendingPaidChapterId),
        );
    ...
    const handleWatchTimeUpdate = useCallback(
        async (seconds: number, duration?: number) => {
            ...
            if (
                now - watchTimeStateLastUpdatedRef.current >=
                WATCH_TIME_STATE_THROTTLE_MS
            ) {
            ...
            if (
                currentChapterId &&
                isUserEnrolled &&
                !autoAdvancedFromChapterRef.current[currentChapterId]
            ) {
                const currentChapterIndex = allChapters.findIndex(
                    (c: any) => String(c.id) === currentChapterId,
                );
                ...
                const reachedUnlockThreshold =
                    effectiveDuration > 0 &&
                    liveSecs >= effectiveDuration * 0.9;

                if (
                    hasNextChapter &&
                    reachedUnlockThreshold &&
                    !autoAdvanceInFlightRef.current[currentChapterId]
                ) {
                    autoAdvanceInFlightRef.current[currentChapterId] = true;
                    void tryAutoAdvanceToNext(currentChapterId, {
                        refreshBeforeCheck: true,
                    }).finally(() => {
                        autoAdvanceInFlightRef.current[currentChapterId] =
                            false;
                    });
            ...
            const contributions = chapters.map((ch: any) => {
                const chapProgress = enrollment?.progress?.find(
                    (p: any) => String(p.chapterId) === String(ch.id),
                );
                const serverWatch = Number(chapProgress?.watchTime ?? 0);
                const serverVideoDuration = Number(
                    (chapProgress as any)?.videoDuration ?? 0,
                );
                ...
                let dur = 0;
                if (
                    String(ch.id) === String(currentChapterRef.current?.id) &&
                    typeof duration === "number" &&
                    duration > 0
                ) {
                    dur = duration;
            ...
            const totalPctSum = contributions.reduce(
                (acc: number, c: any) =>
                    acc + (c.isCompleted ? 100 : Number(c.pct) || 0),
                0,
            );
            const overallCalculated =
                chapters.length > 0 ? totalPctSum / chapters.length : 0;
            ...
            if (refreshNow - lastProgressRefreshRef.current <= THROTTLE_MS)
                return;
            ...
            try {
                ...
                if (status === 429) {
                    ...
                    progressBackoffRef.current.attempts = Math.min(
                        (progressBackoffRef.current.attempts || 0) + 1,
                        6,
                    );
                    const exp = Math.pow(
                        2,
                        progressBackoffRef.current.attempts,
                    );
                    ...
                    const jitter = Math.floor(
                        Math.random() * Math.floor(backoff * 0.5),
                    );
                    const wait = backoff + jitter;
                    progressBackoffRef.current.nextAllowed = refreshNow + wait;
                    lastProgressRefreshRef.current =
                        progressBackoffRef.current.nextAllowed;
                    console.warn(
                        `[Progress Refresh] 429 received - backing off for ${Math.round(wait / 1000)}s (attempt ${progressBackoffRef.current.attempts})`,
                    );
                } else {
                    console.error(
                        "[Progress Refresh] Failed to refresh progress:",
                        err,
                    );
                }
            }
    ...
    const defaultChapterId = useMemo(() => {
        const firstAccessible = allChapters.find((c: any) =>
            isChapterAccessible(String(c.id)),
        );
    ...
    const currentChapter = selectedChapter
        ...
        : defaultChapterId
          ? allChapters.find(
                (c: any) => String(c.id) === String(defaultChapterId),
            )
    ...
    const currentChapterIndex = currentChapter
        ? allChapters.findIndex((c: any) => c.id === currentChapter.id)
        : -1;
    ...
    const progress = useMemo(() => {
        ...
        const chapterProgress = enrollment.progress.find(
            (p: any) => String(p.chapterId) === String(currentChapter.id),
        );
        if (chapterProgress?.isCompleted) return 100;
        // Prefer optimistic overrides when available (live updates from the player)
        const videoDurationFromProgress = Number(
            chapterProgress?.videoDuration ?? 0,
        );
    ...
    const nextChapterId = useMemo(() => {
        ...
        const idx = allChapters.findIndex(
            (c: any) => String(c.id) === String(currentChapter.id),
        );
    ...
    const tryUnlockImmediateNextChapter = useCallback(
        async (targetChapterId: string): Promise<boolean> => {
            ...
            const currentIndex = allChapters.findIndex(
                (c: any) => String(c.id) === currentId,
            );
            if (currentIndex === -1) return false;
            const immediateNext = allChapters[currentIndex + 1];
            if (!immediateNext || String(immediateNext.id) !== targetId)
                return false;

            const currentProgress = Array.isArray(enrollment?.progress)
                ? enrollment.progress.find(
                      (p: any) => String(p.chapterId) === currentId,
                  )
                : null;
            ...
            const effectiveWatch =
                String(currentChapterRef.current?.id || "") === currentId &&
                watchTimeOverride !== null
                    ? Number(watchTimeOverride)
                    : Math.max(storedWatch, localHighWaterWatch);

            const durationFromProgress = Number(
                (currentProgress as any)?.videoDuration ?? 0,
            );
            const durationFromChapter = Number(currentChapter?.duration ?? 0);
            const effectiveDuration =
                String(currentChapterRef.current?.id || "") === currentId &&
                Number(videoDurationOverride ?? 0) > 0
            ...
            const reachedUnlockThreshold =
                ...
                (effectiveDuration > 0 &&
                    effectiveWatch > 0 &&
                    (effectiveWatch / effectiveDuration) * 100 >= 90);
            ...
            try {
                if (!isCompleted) {
                    ...
                    await coursesApi.completeChapterEnrollment(
                        resolvedCourseId,
                        currentId,
                    );
                }
    ...
    const attemptSelectChapter = useCallback(
        async (chapterId: string): Promise<boolean> => {
            const chapter = allChapters.find(
                (c: any) => String(c.id) === String(chapterId),
            );
            if (!chapter) return false;
            ...
            if (!canAccess) {
                ...
                console.info(
                    "[CourseNextFlow] Immediate unlock attempt result",
                    {
                        chapterId: String(chapterId),
                        canAccess,
                    },
            ...
            console.info("[CourseNextFlow] Chapter selected", {
                chapterId: String(chapterId),
            });
            ...
            try {
                if (enrollment) {
                    await coursesApi.startChapter(
                        resolvedCourseId,
                        String(chapter.sectionId),
                        String(chapterId),
                        { watchTime: 0 },
                    );
                }
    ...
    useEffect(() => {
        ...
        const targetChapter = allChapters.find(
            (c: any) => String(c.id) === String(requestedChapterId),
        );
        if (!targetChapter) {
        ...
        if (!isUserEnrolled) {
            console.info(
                "[CourseNextFlow] Requested chapter deferred until enrollment is active",
                {
        ...
        void attemptSelectChapter(String(targetChapter.id))
            ...
            .catch((error) => {
                console.error(
                    "[CourseNextFlow] Requested chapter selection failed",
                    error,
                );
            });
    }, [
        ...
        ? {
              watchTime:
                  ...
                  Number(
                      enrollment?.progress?.find(
                          (p: any) =>
                              String(p.chapterId) === String(currentChapter.id),
                      )?.watchTime ?? 0,
                  ),
              duration:
                  videoDurationOverride ??
    ...
    const displayChapterPercent =
        currentChapterProgressData && currentChapterProgressData.duration > 0
            ? Math.min(
                  (Number(currentChapterProgressData.watchTime) /
                      Number(currentChapterProgressData.duration)) *
                      100,
                  100,
              )
            : progress;
    ...
    const handleEnrollNow = useCallback(async () => {
        ...
        const targetChapterId = currentChapter?.id
            ...
            : undefined;
        const targetChapterPaid =
            Boolean(currentChapter?.isPaidChapter) &&
            !Boolean(currentChapter?.isPreview);
    ...
    useEffect(() => {
        courseSessionRef.current = courseSession;
    }, [courseSession]);
    ...
    const handleSeekVideo = useCallback((seconds: number) => {
        document.dispatchEvent(
            new CustomEvent("chabaqa:seek-to", { detail: { seconds } }),
        );
    }, []);
                    ...
                    <div className="lg:col-span-4 xl:col-span-2 space-y-4">
                                ...
                                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                    <div>
                                        ...
                                        <div className="text-xs text-muted-foreground">
                                            Course completion is auto-recorded.
                                            Use this button only if you want to
                                            retry finalization now.
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <ChapterTabs
                            ...
                            onGoToNextChapter={async () => {
                                console.info(
                                    "[CourseNextFlow] onGoToNextChapter invoked from ChapterTabs",
                                    {
                                        currentChapterId: currentChapter?.id
                                            ? String(currentChapter.id)
                                            : null,
                                ...
                                if (!nextChapterId) {
                                    console.warn(
                                        "[CourseNextFlow] onGoToNextChapter blocked: no next chapter id",
                                    );
                                    return;
                                }
