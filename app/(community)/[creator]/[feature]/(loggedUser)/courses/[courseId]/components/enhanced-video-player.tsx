...
} from "@/lib/api/video-playback.api";
import {
    detectVideoPlatform,
    parseVimeoVideoId,
    parseYouTubeVideoId,
    ...
    return (
        <>
            ...
            <div
                ...
                style={{
                    top: `${(position.top + 45) % 85}%`,
                    left: `${(position.left + 35) % 80}%`,
                }}
            >
                {sessionShort}
            </div>
...
const EnhancedVideoPlayerInner = React.memo(function EnhancedVideoPlayer({
    ...
    const sendWatchTimeRef = useRef<
        (time: number, duration?: number) => Promise<void>
    >(async () => {});
    ...
    const markChapterCompletedOnceRef = useRef<
        (
            reason: string,
            context?: { currentTime?: number; duration?: number },
        ) => Promise<void>
    >(async () => {});
    ...
    const videoUrl = useMemo(() => {
        ...
        if (rawVideoUrl.includes("/uploads/")) {
            try {
                if (
                    rawVideoUrl.startsWith("http://localhost:") ||
                    rawVideoUrl.startsWith("http://127.0.0.1:")
                )
                    return rawVideoUrl;
    ...
    const platform = hasProtectedVideo
        ? playbackSession?.streamType === "hls"
            ? "hls"
            : "local"
        : detectVideoPlatform(videoUrl);
    const youtubeId =
        platform === "youtube" ? parseYouTubeVideoId(videoUrl) : null;
    ...
    const savedWatchPosition = useMemo(() => {
        ...
        const serverPosition = Number(
            (chapterProgress && (chapterProgress as any).watchTime) ?? 0,
        );
    ...
    const chapterAccessible = currentChapter
        ? isChapterAccessible(currentChapter.id)
        : false;
    ...
    const persistHighWaterMark = useCallback(
        (time: number) => {
    ...
    useEffect(() => {
        if (
            savedWatchPosition > 0 &&
            onWatchTimeUpdateRef.current &&
            currentChapter?.id
        ) {
            const chapterProgress = enrollment?.progress?.find(
                (p: any) => String(p.chapterId) === String(currentChapter.id),
            ...
            const duration = Number(
                (chapterProgress && (chapterProgress as any).videoDuration) ||
    ...
    useEffect(() => {
        ...
        const createSession = async () => {
            ...
            try {
                const response = await videoPlaybackApi.createPlaybackSession(
                    courseId,
                    currentChapter.id,
                );
    ...
    useEffect(() => {
        ...
        sessionExtendIntervalRef.current = setInterval(async () => {
            try {
                await videoPlaybackApi.extendSession(playbackSession.sessionId);
            } catch {}
        }, 180000);
        return () => {
    ...
    // Seek support for transcript panel
    useEffect(() => {
        ...
        document.addEventListener("chabaqa:seek-to", handleSeekEvent);
        return () =>
            document.removeEventListener("chabaqa:seek-to", handleSeekEvent);
    }, []);
    ...
    const isLocalFileVideo = useCallback(() => {
        ...
        if (
            /s3\.amazonaws\.com|\.cloudfront\.net|storage\.googleapis\.com|blob\.core\.windows\.net|cdn\./i.test(
                safeUrl,
            )
        )
            return true;
            ...
            try {
                ...
                if (!progressRefreshTriggeredRef.current && onProgressSaved) {
                    ...
                    try {
                        ...
                        console.error(
                            "Failed to refresh progress after watch-time sync:",
                            refreshError,
                        );
                    }
                }
                if (
                    completeRetryPendingRef.current &&
                    completeRetryCountRef.current < 3
                ) {
                    ...
                    try {
                        ...
                        if (
                            typeof (window as any).__onChapterComplete ===
                            "function"
                        ) {
                        ...
                        console.error(
                            "Completion retry after watch-time sync failed:",
                            retryError,
                        );
    ...
    const markChapterCompletedOnce = useCallback(
        async (
            ...
            if (hasSentCompleteRef.current || completeInFlightRef.current)
                return;

            completeInFlightRef.current = true;
            hasSentCompleteRef.current = true;
            try {
                const contextWatchTime = Number(
                    context?.currentTime ?? watchTimeRef.current ?? 0,
                );
                const contextDuration = Number(
                    context?.duration ?? videoDurationRef.current ?? 0,
                );
    ...
    useEffect(() => {
        markChapterCompletedOnceRef.current = markChapterCompletedOnce;
        ...
        const setReadyPlayer = (target: any) => {
            ...
            if (!hasTime || !hasDuration) {
                ...
                if (!youtubeMethodErrorLoggedRef.current) {
                    ...
                    console.warn(
                        "[VideoPlayer] YouTube player missing required methods for tracking",
                    );
                }
                return;
            }
                ...
                events: {
                    ...
                    onStateChange: (event: any) => {
                        ...
                        if (
                            ...
                            setIsPlaying(false);
                            void markChapterCompletedOnceRef.current(
                                "youtube_ended",
                            );
                        } else {
                            setIsPlaying(false);
        ...
        void loadYouTubeIframeApi()
            .then(() => {
                ...
                if (!(window as any).YT?.Player) {
                    if (!youtubeRuntimeErrorLoggedRef.current) {
                        ...
                        console.warn(
                            "[VideoPlayer] YouTube API did not initialize in time",
                        );
                    }
                    return;
                }
            ...
            .catch(() => {
                if (!youtubeRuntimeErrorLoggedRef.current) {
                    youtubeRuntimeErrorLoggedRef.current = true;
                    console.warn(
                        "[VideoPlayer] Failed to initialize YouTube API",
                    );
                }
            });
    ...
    useEffect(() => {
        ...
        const accessAllowed =
            (currentChapter?.id
                ? isChapterAccessible(currentChapter?.id)
                : false) || !!currentChapter?.isPreview;
        ...
        const onTimeUpdate = () => {
            ...
            const effectiveMaxTime = Math.max(
                maxStoredTime,
            ...
            if (duration > 0 && !hasSentCompleteRef.current) {
                if (effectiveMaxTime / duration >= 0.9) {
                    void markChapterCompletedOnceRef.current(
                        "high_watermark_recovery",
                        { currentTime: effectiveMaxTime, duration },
                    );
                }
            }
            ...
            if (
                ...
                Math.floor(currentTime) > maxStoredTime
            ) {
                sendWatchTimeRef.current(
                    currentTime,
                    duration > 0 ? duration : undefined,
                );
            }
        ...
        const onEnded = () => {
            setIsPlaying(false);
            void markChapterCompletedOnceRef.current("native_ended", {
        ...
        if (platform === "youtube" && currentChapter?.id) {
            intervalRef.current = setInterval(async () => {
                ...
                try {
                    ...
                    const canReadTime =
                        typeof youtubeApiRef.current.getCurrentTime ===
                        "function";
                    const canReadDuration =
                        typeof youtubeApiRef.current.getDuration === "function";
                    if (!canReadTime || !canReadDuration) {
                        if (!youtubeMethodErrorLoggedRef.current) {
                            youtubeMethodErrorLoggedRef.current = true;
                            console.warn(
                                "[VideoPlayer] YouTube tracking skipped: missing getCurrentTime/getDuration",
                            );
                        }
                        return;
                    }
                    ...
                    const currentTime = Number(
                        (await youtubeApiRef.current.getCurrentTime()) || 0,
                    );
                    const duration = Number(
                        (await youtubeApiRef.current.getDuration()) || 0,
                    );
                    youtubeRuntimeErrorLoggedRef.current = false;
                    youtubePollingBackoffUntilRef.current = 0;
                    ...
                    const shouldEmitUiUpdate =
                        isAdvancing ||
                        flooredTime > lastEmbeddedUiEmitRef.current;
                    ...
                    if (
                        (embeddedSyncStartedRef.current || isAdvancing) &&
                        currentTime - lastUpdateRef.current >= 5
                    ) {
                        const maxStored = getStoredHighWaterMark();
                    ...
                    if (duration > 0 && !hasSentCompleteRef.current) {
                        ...
                        if (pct >= 0.9) {
                            await markChapterCompletedOnceRef.current(
                                "interval_embedded_90_percent",
                                { currentTime, duration },
                            );
                        }
                    }
                    ...
                    if (!youtubeRuntimeErrorLoggedRef.current) {
                        youtubeRuntimeErrorLoggedRef.current = true;
                        console.warn(
                            "[VideoPlayer] YouTube polling paused after runtime error",
                        );
                    }
                }
            }, 1000);
    ...
    useEffect(() => {
        if (!courseId || !currentChapter?.id || !watchTime || !enrollment)
            return;
        ...
        const saveFinalProgress = () => {
            ...
            const payload = JSON.stringify({
                ...
                videoDuration:
                    videoDuration > 0 ? Math.floor(videoDuration) : undefined,
            });
    ...
    const shouldShowLocked =
        ...
        !videoUrl ||
        Boolean(playerError) ||
        (!chapterAccessible && !currentChapter?.isPreview);
    ...
    const effectiveShowLocked =
        hasProtectedVideo && playbackSession ? false : shouldShowLocked;

    if (effectiveShowLocked) {
        const isAccessDenied = !chapterAccessible && !currentChapter?.isPreview;
        const isVideoMissing =
            !hasValidVideoUrl || !videoUrl || Boolean(playerError);
                            ...
                            {isVideoMissing ? (
                                <>
                                    ...
                                    <p className="text-sm text-gray-300 mt-2">
                                        {playerError ||
                                            "The video for this chapter is currently unavailable."}
                                    </p>
                                    <p className="text-xs text-gray-400 mt-1">
                                        Please contact the course creator to
            ...
            Browser/native fullscreen can elevate media layers above DOM overlays. */}
                ...
                {playbackSession?.watermark && (
                    <div className="absolute inset-0 pointer-events-none z-20 overflow-hidden select-none">
                        <WatermarkText
                            text={playbackSession.watermark.text}
                            sessionShort={
                                playbackSession.watermark.sessionShort
                            }
                        />
                    </div>
                )}
                    ...
                    watchTime > 0 && (
                        <div className="absolute top-2 right-2 z-30 bg-black/70 text-white text-xs px-2 py-1 rounded flex items-center gap-2">
                            ...
                            <span>
                                ...
                                {String(Math.floor(watchTime % 60)).padStart(
                                    2,
                                    "0",
                                )}
                                {videoDuration > 0 &&
