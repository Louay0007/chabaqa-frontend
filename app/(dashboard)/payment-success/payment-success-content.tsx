...
export default function PaymentSuccessContent() {
    ...
    useEffect(() => {
        const verifyPayment = async () => {
            ...
            try {
                ...
                // For subscription payments, the backend webhook handles activation —
                // no verification endpoint is needed.
                ...
                const isSuccess =
                    data?.success === true || Boolean(response?.ok);
                const payload = data.data || data;
                const status = payload?.status;
                const action = payload?.action;
                const errorMessage =
                    payload?.message || data?.message || data?.error;
                ...
                if (
                    ...
                    if (!duplicateEventToastShown.current) {
                        ...
                        toast({
                            ...
                            description:
                                "You can only register once per event (1 ticket per user).",
                        });
                    }
                } else if (
                    isSuccess &&
                    ...
                    const sessionTargetId =
                        payload?.sessionContentId || payload?.targetId || id;
                    const redirectUrl =
                        payload?.creatorSlug &&
                    ...
                    toast({
                        ...
                        description:
                            payload?.message ||
                            "Please choose a date and time to finalize your booking.",
                    });
    ...
    useEffect(() => {
        ...
        if (scope === "course" && creatorSlug && communitySlug && targetId) {
            ...
            router.replace(
                `/${creatorSlug}/${communitySlug}/courses/${targetId}`,
            );
            return;
        }
        ...
        if (scope === "chapter" && creatorSlug && communitySlug) {
            ...
            if (courseTargetId) {
                ...
                if (chapterId)
                    nextParams.set("paidChapterId", String(chapterId));
                nextParams.set("checkout", "success");
                if (sessionId) nextParams.set("sessionId", String(sessionId));
                const query = nextParams.toString();
                router.replace(
                    `/${creatorSlug}/${communitySlug}/courses/${courseTargetId}${query ? `?${query}` : ""}`,
                );
                return;
            }
        }
            ...
            (async () => {
                ...
                try {
                    ...
                    if (joined) {
                        ...
                        router.replace(
                            `/${creatorSlug}/${communitySlug}/home?joined=1`,
                        );
                    }
                }
            })();
        ...
        // Subscription Redirect — short delay so the user sees the success state
        if (scope === "subscription") {
            redirectDone.current = true;
            const timer = setTimeout(() => {
                router.replace("/creator/plan");
            }, 1500);
    ...
    const renderContentButton = () => {
        const baseClass =
            "block w-full bg-blue-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-blue-700 transition text-center";

        // 1. Course
        if (scope === "course" && creatorSlug && communitySlug && targetId) {
            return (
                <Link
                    href={`/${creatorSlug}/${communitySlug}/courses/${targetId}`}
                    className={baseClass}
                >
                    Go to Course
                </Link>
        ...
        if (scope === "chapter" && creatorSlug && communitySlug) {
            ...
            if (courseTargetId) {
                ...
                return (
                    <Link
                        href={`/${creatorSlug}/${communitySlug}/courses/${courseTargetId}${query ? `?${query}` : ""}`}
                        className={baseClass}
                    >
                        Go to Course
                    </Link>
        ...
        if (scope === "community" && creatorSlug && communitySlug) {
            return (
                <Link
                    href={`/${creatorSlug}/${communitySlug}/home`}
                    className={baseClass}
                >
                    Go to Community
                </Link>
        ...
        if (scope === "product" && creatorSlug && communitySlug && targetId) {
            return (
                <Link
                    href={`/${creatorSlug}/${communitySlug}/products/${targetId}`}
                    className={baseClass}
                >
                    Go to Product
                </Link>
        ...
        if (scope === "challenge" && creatorSlug && communitySlug && targetId) {
            return (
                <Link
                    href={`/${creatorSlug}/${communitySlug}/challenges/${targetId}`}
                    className={baseClass}
                >
                    Go to Challenge
                </Link>
        ...
        if (scope === "session") {
            if (creatorSlug && communitySlug) {
                return (
                    <Link
                        href={`/${creatorSlug}/${communitySlug}/sessions`}
                        className={baseClass}
                    >
                        View My Bookings
                    </Link>
        ...
        if (scope === "subscription") {
            return (
                <Link href="/creator/plan" className={baseClass}>
                    Go to My Plan
                </Link>
            );
        }
                    ...
                    {loading ? (
                        ...
                        <div className="text-center">
                            ...
                            <p className="text-gray-600 mb-6">
                                Your payment was successful and your access has
                                been enabled.
                                {contentTitle && (
