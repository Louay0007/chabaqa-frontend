...
export function ChallengeResourceCard({
    ...
    const youtubeEmbed =
        resource.type === "video" ? getYoutubeEmbedUrl(resource.url) : null;
    const vimeoEmbed =
        resource.type === "video" ? getVimeoEmbedUrl(resource.url) : null;
    const isDirectVideo =
        resource.type === "video" ? isDirectVideoUrl(resource.url) : false;
    ...
    const uploadVideoResource = async (file: File) => {
        ...
        try {
            ...
            toast({
                title: "Upload failed",
                description:
                    error?.message || "Unable to upload video. Try again.",
                variant: "destructive",
            });
        } finally {
        ...
        <div className="border rounded-lg p-3 bg-white">
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center space-x-2">
                    ...
                    <Input
                        ...
                        onChange={(e) =>
                            updateResource(resIndex, "title", e.target.value)
                        }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="space-y-1">
                    ...
                    <Select
                        value={resource.type}
                        onValueChange={(value) =>
                            updateResource(resIndex, "type", value)
                        }
                    >
                        <SelectTrigger
                            className={`h-8 text-sm ${getError?.("type") ? "border-red-500" : ""}`}
                        >
                            <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                ...
                <div className="space-y-1">
                    ...
                    <Input
                        ...
                        value={resource.url}
                        onChange={(e) =>
                            updateResource(resIndex, "url", e.target.value)
                        }
            ...
            <div className="space-y-1 mt-3">
                ...
                <Textarea
                    ...
                    onChange={(e) =>
                        updateResource(resIndex, "description", e.target.value)
                    }
                    rows={1}
                    className="text-sm"
                />
                <WanisAssistButton
                    value={resource.description}
                    onAccept={(newText) =>
                        updateResource(resIndex, "description", newText)
                    }
                    context="Challenge resource description — a brief summary of what this learning resource covers"
                />
            </div>
                ...
                <div className="mt-3 rounded-md border bg-black overflow-hidden">
                    {youtubeEmbed || vimeoEmbed ? (
                        ...
                        <div className="p-3 text-xs text-muted-foreground bg-muted">
                            Preview unavailable for this video URL. It will
                            still be saved as a resource link.
                        </div>
                    )}
                </div>
            )}
