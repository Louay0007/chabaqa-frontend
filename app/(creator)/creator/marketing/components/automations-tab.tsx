...
function WelcomeEmailSection() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            const t =
                await emailCampaignsApi.getWelcomeTemplate(selectedCommunityId);
            setTemplate(t);
        } catch {
            setTemplate(null);
        } finally {
    ...
    const openForm = () => {
        setFormSubject(
            template?.subject ?? `Welcome to {{communityName}}, {{userName}}!`,
        );
        setFormContent(
            template?.content ??
    ...
    const saveForm = async () => {
        ...
        try {
            ...
            if (isEdit && template) {
                saved = await emailCampaignsApi.updateWelcomeTemplate(
                    selectedCommunityId,
                    {
                ...
                saved = await emailCampaignsApi.createWelcomeTemplate(
                    selectedCommunityId,
                    {
                        subject: formSubject.trim(),
                        content: formContent,
            ...
            toast({
                ...
                description:
                    "It will be sent automatically to every new member.",
            });
        } catch (err: any) {
            toast({
                title: "Error",
                description: err?.message ?? "Failed to save welcome email",
                variant: "destructive",
            });
        } finally {
            setFormSaving(false);
    ...
    const toggleActive = async (active: boolean) => {
        ...
        setToggling(true);
        try {
            const updated = await emailCampaignsApi.toggleWelcomeTemplate(
                selectedCommunityId,
                active,
            );
            setTemplate(updated);
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <CardDescription className="text-sm mt-0.5">
                                    Automatically sent to every new member the
                                    moment they join your community. Supports{" "}
                        ...
                        {loading ? (
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0">
                                <Switch
                                    checked={
                                        template.automationActive !== false
                                    }
            ...
            <Dialog open={showForm} onOpenChange={setShowForm}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            This email is sent automatically when someone joins{" "}
                            <strong>{selectedCommunity?.name}</strong>. Use{" "}
                            <code>{"{{userName}}"}</code> and{" "}
                            <code>{"{{communityName}}"}</code> as placeholders.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4">
                        <div>
                            <Label htmlFor="welcome-subject">
                                Subject line
                            </Label>
                            <Input
                                id="welcome-subject"
                        ...
                        <div>
                            ...
                            <WanisAssistButton
                                value={formContent}
                                onAccept={(newText) => setFormContent(newText)}
                                context="Welcome email content sent automatically to new community members"
                            />
                        </div>
                        <div className="flex items-center gap-3">
                            <Switch
            ...
            >
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            New members will no longer receive an automated
                            welcome email. This cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
...
function InactivityAutomationCard({
    ...
    const { toast } = useToast();
    const days =
        automation.metadata?.minInactiveDays ??
        automation.targetDaysThreshold ??
        "?";
        ...
        <div className="flex items-start gap-3 rounded-lg border p-4 bg-white">
            ...
            <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                    Triggers when a member is inactive for{" "}
...
function InactivityAutomationsSection() {
    ...
    const load = useCallback(async () => {
        ...
        try {
            const list =
                await emailCampaignsApi.getInactivityAutomations(
                    selectedCommunityId,
                );
    ...
    const handleToggle = async (id: string, active: boolean) => {
        const updated = await emailCampaignsApi.toggleInactivityAutomation(
            id,
            active,
        );
        setAutomations((prev) =>
                    ...
                    <div className="flex items-start justify-between gap-4">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <CardDescription className="text-sm mt-0.5">
                                    Automatically send a re-engagement email
                                    when a member hasn't visited for N days. The
                                    system checks daily and respects a 30-day
                                    cooldown per member. Supports{" "}
                                    <code className="text-xs bg-gray-100 px-1 rounded">
                                        {"{{daysThreshold}}"}
                                    </code>
                                    .
                    ...
                    {loading ? (
                        ...
                        <div className="text-center py-8 text-gray-400 border-2 border-dashed rounded-lg">
                            ...
                            <p className="text-sm font-medium">
                                No inactivity rules yet
                            </p>
                            <p className="text-xs mt-1">
                                Add a rule to automatically reach inactive
                                members.
                            </p>
                        </div>
                    ) : (
            ...
            <Dialog open={showForm} onOpenChange={setShowForm}>
                <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            This email will be sent automatically every day to
                            members whose last visit was exactly{" "}
                            <strong>N days ago</strong>. Each member can receive
                            it at most once every 30 days.
                        </DialogDescription>
                    </DialogHeader>
                    <div className="space-y-4">
                        <div className="grid grid-cols-3 gap-4">
                            <div className="col-span-2">
                                ...
                                <Input
                                    ...
                                    placeholder='e.g. "We miss you — 15 days"'
                                    className="mt-1"
                                />
                            </div>
                            <div>
                        ...
                        <div>
                            ...
                            <WanisAssistButton
                                value={formContent}
                                onAccept={(newText) => setFormContent(newText)}
                                context="Inactivity re-engagement email content sent to members who have been inactive"
                            />
                        </div>
                        <div className="flex items-center gap-3">
                            <Switch
            ...
            <div className="flex items-center gap-3 p-4 rounded-xl bg-gradient-to-r from-chabaqa-primary/5 to-purple-50 border border-chabaqa-primary/10">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500">
                        Set-and-forget emails that fire automatically based on
                        member behaviour — no manual work needed.
                    </p>
                </div>
            </div>
