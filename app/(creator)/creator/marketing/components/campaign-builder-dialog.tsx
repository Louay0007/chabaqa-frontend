...
const VARIABLE_DEFS: Array<{
    ...
    },
    {
        ...
        description: "Your community’s name",
        showFor: ["base"],
    },
    {
        key: "currentDate",
    ...
    },
    {
        ...
        description: "Days since last login (inactive campaigns)",
        showFor: ["inactive"],
    },
    {
        key: "inactivityPeriod",
        label: "Inactivity period",
        description: "Selected inactive period label",
        showFor: ["inactive"],
    },
    {
        key: "contentTypeLabel",
        label: "Content type label",
        description: "Selected content type label (content reminder)",
        showFor: ["content"],
    },
    {
        key: "contentType",
        label: "Content type",
        description: "Selected content type key",
        showFor: ["content"],
    },
    {
        key: "progressPct",
        label: "Progress %",
        description: "Learner's current course completion %",
        showFor: ["course"],
    },
    {
        key: "enrolledDays",
        label: "Enrolled days",
        description: "Days since the learner enrolled",
        showFor: ["course"],
    },
];
...
const inactivityPeriodDaysThreshold = (
...
export function CampaignBuilderDialog(props: {
    ...
    const [inactiveStats, setInactiveStats] =
        useState<InactiveUserStats | null>(null);
    const [inactiveStatsLoading, setInactiveStatsLoading] = useState(false);
    const [inactiveStatsError, setInactiveStatsError] = useState<string | null>(
        null,
    );

    const [contentItems, setContentItems] = useState<ContentPick[]>([]);
    const [contentItemsLoading, setContentItemsLoading] = useState(false);
    const [contentItemsError, setContentItemsError] = useState<string | null>(
        null,
    );
    ...
    const [activeField, setActiveField] = useState<"subject" | "content">(
        "subject",
    );
    ...
    useEffect(() => {
        ...
        emailCampaignsApi
            ...
            .catch((error: any) => {
                ...
                setInactiveStatsError(
                    typeof error?.message === "string"
                        ? error.message
                        : "Failed to load recipient estimate.",
                );
            })
            .finally(() => {
                if (cancelled) return;
                ...
                const normalize = (items: any[]): ContentPick[] =>
                    items
                        .map((it) => ({
                            ...
                            title: String(
                                it?.title || it?.name || it?.titre || "",
                            ).trim(),
                        }))
                        .filter((it) => it.id && it.title);
                if (!cancelled)
                    setCourseItems(normalize(extractList(response)));
            } catch {
                if (!cancelled) setCourseItems([]);
            } finally {
    ...
    const handlePreviewCourseAudience = useCallback(async () => {
        ...
        try {
            ...
            toast({
                ...
                description: err?.message ?? "Could not estimate audience",
                variant: "destructive",
            });
        } finally {
            setPreviewingAudience(false);
    ...
    const recipientsEstimate = useMemo(() => {
        ...
        if (kind === "inactive-users") {
            if (inactivityPeriod === "last_7_days")
                return inactiveStats.inactive7d;
            if (inactivityPeriod === "last_15_days")
                return inactiveStats.inactive15d;
            if (inactivityPeriod === "last_30_days")
                return inactiveStats.inactive30d;
            if (inactivityPeriod === "last_60_days")
                return inactiveStats.inactive30d;
            if (inactivityPeriod === "more_than_60_days")
                return inactiveStats.inactive60dPlus;
            return null;
        }
    ...
    const availableVariables = useMemo(() => {
        ...
        return VARIABLE_DEFS.filter((v) =>
            v.showFor.some((b) => buckets.includes(b)),
        );
    }, [kind]);
    ...
    const previewVariables = useMemo(() => {
        ...
        return {
            ...
            daysThreshold: inactivityPeriodDaysThreshold(
                inactivityPeriod || undefined,
            ),
            inactivityPeriod: inactivityPeriodLabel(
                inactivityPeriod || undefined,
            ),
        ...
        (async () => {
            try {
                ...
                if (contentType === "event") {
                    ...
                    const response =
                        await productsApi.getByCommunity(communityId);
                    list = normalize(extractList(response));
    ...
    const insertVariableAtCursor = useCallback(
        (key: string) => {
            const token = `{{${key}}}`;
            const targetField =
                activeField === "content" ? "content" : "subject";
            const el =
                targetField === "content"
                    ? contentRef.current
                    : subjectRef.current;
    ...
    const validateStep = useCallback((): string | null => {
        ...
        if (step === 1) {
            ...
            if (kind === "inactive-users" && !inactivityPeriod)
                return "Pick an inactivity period.";
            if (kind === "content-reminder" && !contentType)
                return "Pick a content type.";
            if (kind === "course-progress") {
                if (!targetCourseId) return "Select a course to target.";
                const maxPct = parseInt(targetMaxProgressPct, 10);
                const minDays = parseInt(targetMinEnrolledDays, 10);
                if (isNaN(maxPct) || maxPct < 1 || maxPct > 99)
                    return "Max progress must be between 1 and 99%.";
                if (isNaN(minDays) || minDays < 1)
                    return "Enrolled days must be at least 1.";
            }
            return null;
        }
        ...
        try {
            ...
            if (kind === "content-reminder") {
                const response = await emailCampaignsApi.createContentReminder({
                    ...
                    trackOpens,
                    trackClicks,
                    metadata: contentLabelValue
                        ? { contentLabel: contentLabelValue }
                        : undefined,
                } as any);
                ...
                toast({
                    ...
                    description: scheduledAt
                        ...
                        : "Your campaign was queued for sending.",
                });
                onOpenChange(false);
                onSuccess?.();
                return response;
            ...
            if (kind === "course-progress") {
                const created =
                    await emailCampaignsApi.createCourseProgressCampaign({
                        title: title.trim(),
                        subject: subject.trim(),
                        content,
                        communityId: selectedCommunityId,
                        targetCourseId,
                        targetMaxProgressPct: parseInt(
                            targetMaxProgressPct,
                            10,
                        ),
                        targetMinEnrolledDays: parseInt(
                            targetMinEnrolledDays,
                            10,
                        ),
                        scheduledAt,
                ...
                if (!scheduledAt && sendingTime === "now") {
                    try {
                        ...
                        toast({
                            ...
                            description:
                                "Course progress reminder queued for sending.",
                        });
                    } catch (sendError: any) {
                        toast({
                ...
                if (!scheduledAt && sendingTime === "now") {
                    try {
                        ...
                        toast({
                            ...
                            description:
                                sendError?.message ||
                                "Campaign created as draft. You can send it from the list.",
                            variant: "destructive",
                        });
                    }
                } else {
    ...
    const dialogDescription = initialValues
        ? "Review and customize this campaign before sending."
        : "Build a campaign for your community members.";

    const primaryCtaLabel =
        sendingTime === "scheduled" ? "Schedule Campaign" : "Create & Send";
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
                        <div className="lg:col-span-3 space-y-4">
                                    ...
                                    {[1, 2, 3].map((n) => (
                                        <div
                                            ...
                                            className={cn(
                                                ...
                                                step === n
                                                    ? "border-chabaqa-primary bg-chabaqa-primary/10 text-chabaqa-primary"
                            ...
                            {step === 1 ? (
                                <div className="space-y-4">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                                            <button
                                                ...
                                                className={cn(
                                                    ...
                                                    kind === "announcement"
                                                        ? "border-chabaqa-primary bg-chabaqa-primary/5"
                                                        : "border-gray-200",
                                                )}
                                                onClick={() =>
                                            ...
                                            >
                                                ...
                                                <p className="text-xs text-gray-600 mt-1">
                                                    Remind learners who haven't
                                                    completed a course yet.
                                                </p>
                                            </button>
                                            <button
                                                type="button"
                                    ...
                                    {kind === "inactive-users" ? (
                                        <div className="space-y-2">
                                            <Label htmlFor="campaign-inactivity-period">
                                                Inactive period
                                            </Label>
                                            <Select
                                    ...
                                    {kind === "content-reminder" ? (
                                        <div className="space-y-3">
                                            ...
                                            contentType !== "all" ? (
                                                <div className="space-y-2">
                                                    <Label htmlFor="campaign-content-item">
                                                    ...
                                                    {contentItemsError ? (
                                                        <div className="rounded-md border border-amber-200 bg-amber-50 p-3">
                                                            ...
                                                            <div className="mt-2 space-y-2">
                                                                <Label
                                                                    htmlFor="campaign-content-id-fallback"
                                            ...
                                            <div className="grid grid-cols-2 gap-4">
                                                <div className="space-y-1.5">
                                                    ...
                                                    <p className="text-xs text-gray-400">
                                                        Email learners below
                                                        this completion
                                                    </p>
                                                </div>
                                                <div className="space-y-1.5">
                                                    <Label htmlFor="cp-min-days">
                                                        Min enrolled days
                                                    </Label>
                                                    <Input
                                                        id="cp-min-days"
                            ...
                            {step === 2 ? (
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <p className="text-xs text-gray-500">
                                            Click to insert into the focused
                            ...
                            {step === 3 ? (
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        <Label htmlFor="campaign-sending-time">
                                            When to send
                                        </Label>
                                        <Select
                                            value={sendingTime}
                                            onValueChange={(v) =>
                                                setSendingTime(v as any)
                                            }
                                        >
                                    ...
                                    {sendingTime === "scheduled" ? (
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                            <div className="space-y-2">
                                                <Label htmlFor="campaign-scheduled-date">
                                                    Date
                                                </Label>
                                                <Input
                                            ...
                                            <div className="space-y-2">
                                                <Label htmlFor="campaign-scheduled-time">
                                                    Time
                                                </Label>
                                                <Input
                                        ...
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            inactivityPeriod ? (
                                                <Badge variant="secondary">
                                                    Inactive:{" "}
                                                    {inactivityPeriodLabel(
                                                        inactivityPeriod as any,
                                                    )}
                                                </Badge>
                                            ) : null}
                                            {kind === "content-reminder" &&
                            ...
                            <div className="flex items-center justify-between pt-2">
                                ...
                                {kind === "custom-invitation" ? null : step <
                                  3 ? (
                                    <Button
                                        type="button"
                                        onClick={goNext}
                        ...
                        <div className="lg:col-span-2 space-y-3">
                            <div className="rounded-lg border bg-white p-4">
                                ...
                                <p className="text-xs text-gray-500 mt-1">
                                    Preview uses sample values (Test User, your
                                    community, today).
                                </p>
                                    ...
                                    <div className="rounded-md border bg-gray-50 p-3">
                                        ...
                                        {isHtml ? (
                                            <iframe
                                                ...
                                                srcDoc={
                                                    previewContent ||
                                                    "<div style='padding:12px;color:#6b7280;'>No content yet.</div>"
                                                }
                                            />
                                        ) : (
                            ...
                            <div className="rounded-lg border bg-white p-4">
                                ...
                                <ul className="mt-2 space-y-2 text-xs text-gray-600">
                                    <li>
                                        Keep your subject short and clear (40–60
                                        chars).
                                    </li>
                                    <li>
                                        Start with a promise, then a single CTA.
                                    </li>
                                    <li>
                                        Use variables to personalize at scale.
                                    </li>
                                </ul>
                            </div>
                        </div>
