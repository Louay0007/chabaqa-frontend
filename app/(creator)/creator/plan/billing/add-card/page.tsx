...
// ---------------------------------------------------------------------------
function CardElementForm({ onSuccess }: { onSuccess: (card: { brand: string; last4: string }) => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        toast({ title: 'Card saved!', description: `${result.card.brand?.toUpperCase()} •••• ${result.card.last4}` });
        onSuccess(result.card);
      }
    } catch (err: any) {
      toast({ title: 'Card setup failed', description: err?.message || 'Try again.', variant: 'destructive' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="rounded-md border p-4 bg-white">
        <CardElement options={{ style: { base: { fontSize: '16px', color: '#1a1a1a', '::placeholder': { color: '#9ca3af' } }, invalid: { color: '#ef4444' } }, hidePostalCode: true }} />
      </div>
      ...
      <Button type="submit" className="w-full" disabled={!stripe || loading}>
        {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Saving...</> : <><CreditCard className="h-4 w-4 mr-2" />Save Card</>}
      </Button>
    </form>
  );
}
...
function RedirectForm() {
  ...
  const handleRedirect = async () => {
    ...
    try {
      ...
      toast({ title: 'Error', description: err?.message || 'Failed to start card setup.', variant: 'destructive' });
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start gap-2 rounded-md bg-blue-50 p-3 text-sm text-blue-800">
        <Lock className="h-4 w-4 shrink-0 mt-0.5" />
        <p>You&apos;ll be redirected to a secure Stripe page to enter your card details. Chabaqa never stores your full card number.</p>
      </div>
      <Button className="w-full" onClick={handleRedirect} disabled={loading}>
        {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Redirecting...</> : <><CreditCard className="h-4 w-4 mr-2" />Add Card via Stripe</>}
      </Button>
    </div>
  );
}
...
// ---------------------------------------------------------------------------
function SetupSuccessHandler({ onSuccess }: { onSuccess: (card: { brand: string; last4: string }) => void }) {
  ...
  useEffect(() => {
    if (setupParam === 'success' && sessionId) {
      paymentMethodApi.completeSetupSession(sessionId)
        .then(result => { if (result.success) onSuccess(result.card); })
        .catch(() => toast({ title: 'Error', description: 'Failed to finalize card.', variant: 'destructive' }));
    }
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex flex-col items-center text-center py-6">
              ...
              <p className="text-muted-foreground mb-1">{savedCard.brand?.toUpperCase()} •••• {savedCard.last4}</p>
              <p className="text-sm text-muted-foreground">This card will be used for your subscription billing.</p>
  ...
  return (
    <div className="max-w-md mx-auto space-y-6">
      <Suspense fallback={null}>
        <SetupSuccessHandler onSuccess={handleSuccess} />
      </Suspense>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Save a card for automatic subscription billing.</CardDescription>
        </CardHeader>
        <CardContent>
          {hasPublishableKey === null ? (
            ...
            <Elements stripe={stripePromise}>
              <CardElementForm onSuccess={handleSuccess} />
            </Elements>
          ) : (
            <RedirectForm />
          )}
