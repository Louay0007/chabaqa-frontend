...
export function CommunitiesSearchSection({ items }: CommunitiesSearchSectionProps) {
  ...
  return (
    <section className="pb-16 px-6 md:px-10 bg-[var(--bg)]" aria-label={t('gridLabel')}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-4">
          <p className="text-sm text-[var(--t3)]">
            ...
            {t('resultsOf')}{' '}
            <span className="font-bold text-[var(--t1)]">{items.length}</span>{' '}
            {t('results')}
          </p>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between mt-10 flex-wrap gap-4" role="navigation" aria-label="Pagination">
                <span className="text-sm text-[var(--t3)]">
                  Page <span className="font-bold text-[var(--t1)]">{page}</span> of <span className="font-bold text-[var(--t1)]">{totalPages}</span>
                </span>
                    ...
                    n < 0 ? (
                      ...
                      <button key={n} onClick={() => setPage(n)} aria-current={page === n ? 'page' : undefined}
                        className={`w-9 h-9 flex items-center justify-center rounded-xl text-sm font-bold border transition-all ${
                          page === n ? 'bg-[var(--p)] text-white border-[var(--p)]' : 'bg-[var(--white)] text-[var(--t2)] border-[var(--bd)] hover:border-[var(--p3)] hover:text-[var(--p)]'
                        }`}>
                        {n}
                      </button>
                    )
