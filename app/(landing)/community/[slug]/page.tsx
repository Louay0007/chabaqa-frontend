...
import { CommunityHomePageRenderer } from "./components/community-home-renderer";
...
export async function generateMetadata({
    params,
}: CommunityDetailsPageProps): Promise<Metadata> {
        ...
        typeof rawMembers === "number"
            ...
            : Array.isArray(rawMembers)
              ...
              : typeof rawMembers === "object" &&
                  rawMembers !== null &&
                  typeof rawMembers.count === "number"
                ? rawMembers.count
                : 0;
...
function normalizePricing(community: any): {
    price: number;
    priceType: string;
} {
...
function normalizePageContent(
    ...
    const mapAsset = (value?: string) =>
        value ? resolveImageUrl(value, assetBase) : value;
    ...
    return {
        ...
        testimonials: content.testimonials
            ? {
                  ...
                  testimonials: (content.testimonials.testimonials || []).map(
                      (testimonial) => ({
                          ...testimonial,
                          avatar:
                              mapAsset(testimonial.avatar) ||
                              testimonial.avatar,
                      }),
                  ),
              }
        ...
        cta: content.cta
            ? {
                  ...content.cta,
                  customBackground:
                      mapAsset(content.cta.customBackground) || "",
              }
            : content.cta,
    };
}
...
async function fetchLatestCommunityPosts(
    ...
    try {
        const res = await fetch(
            `${apiBase}/posts/community/${communityId}?page=1&limit=6`,
            {
                method: "GET",
                cache: "force-cache",
                next: {
        ...
        const posts = Array.isArray(postsContainer)
            ? postsContainer
...
/**
 * Fetch the new landing-page-backed community home page
 */
async function fetchCommunityHomePage(
    apiBase: string,
    slug: string,
): Promise<{ source: string; data: any } | null> {
    try {
        const res = await fetch(`${apiBase}/communities/${slug}/home-page`, {
            cache: "force-cache",
            next: { revalidate: PUBLIC_COMMUNITY_DATA_REVALIDATE_SECONDS },
        });
        if (!res.ok) return null;
        const json = await res.json();
        if (
            json?.success &&
            json?.source === "landing-page" &&
            json?.data?.page
        ) {
            return json;
        }
...
export default async function CommunityDetailsPage({
    ...
    const rawCreator =
        (community as any)?.creator || (community as any)?.createur || null;
    const creatorObject =
        rawCreator && typeof rawCreator === "object" ? rawCreator : null;
    const creatorNameFallback =
        typeof rawCreator === "string"
            ? rawCreator
            : String(
                  (community as any)?.creatorName ||
                      (community as any)?.createurName ||
                      "",
              ).trim();
    ...
    const rawRating = asNumber(
        (community as any)?.averageRating ?? (community as any)?.rating,
        0,
    );
    ...
    const [ratingsSnapshot, rawPageContent, latestPosts, homePage] =
        await Promise.all([
            withTimeoutFallback(
    ...
    const communityData = {
        ...
        creator:
            creatorObject || creatorNameFallback
                ? {
                      id: String(
                          (creatorObject as any)?._id ||
                              creatorObject?.id ||
                              "",
                      ),
                      name: String(
                          (creatorObject as any)?.name ||
                              creatorNameFallback ||
                              "Unknown Creator",
                      ),
                      avatar:
                          resolveImageUrl(creatorAvatarRaw, apiBaseForImages) ||
                          undefined,
        ...
        longDescription: normalizeDisplayText(
            (community as any).longDescription,
        ),
    ...
    const checkoutCommunityData = {
        ...
        pricing: {
            price: asNumber(
                (communityData as any).pricing?.price,
                asNumber((communityData as any).price, 0),
            ),
            currency: String(
                (communityData as any).pricing?.currency ||
                    (communityData as any).currency ||
                    "TND",
            ),
        },
        ...
        creatorBankDetails: {
            ownerName: String(
                (communityData as any)?.creatorBankDetails?.ownerName || "",
            ),
            bankName: String(
                (communityData as any)?.creatorBankDetails?.bankName || "",
            ),
    ...
    const overviewContent =
        pageContent?.overview && pageContent.overview.visible !== false
            ? pageContent.overview
            : null;
    const benefitsContent =
        pageContent?.benefits && pageContent.benefits.visible !== false
    ...
    const testimonialsContent =
        pageContent?.testimonials && pageContent.testimonials.visible !== false
            ...
            : null;
    const ctaContent =
        pageContent?.cta && pageContent.cta.visible !== false
            ? pageContent.cta
            : null;

    const pageContentFeatures = (overviewContent?.cards || [])
        .filter(
            (card) => card.visible !== false && typeof card.title === "string",
        )
        .sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
        .map((card) => card.title);
    const effectiveFeatures = dedupeTextList(
        normalizedSettings.features.length > 0
    ...
    const pageContentBenefits = (benefitsContent?.benefits || [])
        .filter(
            (benefit) =>
                benefit.visible !== false && typeof benefit.title === "string",
        )
        .sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
        .map((benefit) => benefit.title);
    const effectiveBenefits = dedupeTextList(
        normalizedSettings.benefits.length > 0
            ? normalizedSettings.benefits
            : pageContentBenefits,
    );

    const effectiveWelcomeMessage =
        normalizedSettings.welcomeMessage?.trim() ||
    ...
    const overviewSubtitle =
        ...
        "Everything you need to succeed is included in this community.";

    // If a landing-page-backed home page exists, use the new renderer
    if (
        homePage?.source === "landing-page" &&
        homePage?.data?.page?.blocks?.length > 0
    ) {
        return (
            <div className="min-h-screen bg-white">
                <CommunityHomePageRenderer
                    community={communityData}
                    blocks={homePage.data.page.blocks}
            ...
            <main className="relative isolate overflow-hidden">
                <div className="pointer-events-none absolute inset-0 -z-10">
                    ...
                    <div
                        ...
                        style={{
                            backgroundColor: themeTokens.softSecondary,
                            opacity: 0.35,
                        }}
                    />
                    <div
                ...
                {shouldRenderOverview && (
                    <section
                        className={cn(
                            "mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12",
                            contentWidthClass,
                        )}
                    >
                        <div
