...
export function TrialBanner() {
    ...
    const timeText =
        trial.remaining.days > 0
            ? `${trial.remaining.days} day${trial.remaining.days !== 1 ? "s" : ""}`
            : `${trial.remaining.hours}h ${trial.remaining.minutes}m`;

    // Calculate progress: days elapsed out of 7-day trial
    const totalTrialDays = 7;
    const daysElapsed = totalTrialDays - trial.remaining.days;
    const progressPercent = Math.min(
        100,
        Math.max(0, (daysElapsed / totalTrialDays) * 100),
    );
    ...
    if (hasPayment) {
        return (
            <div className="flex flex-col gap-2 px-4 py-2.5 text-sm bg-green-50 text-green-800 border-b border-green-200">
                <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                        <CheckCircle className="h-4 w-4 shrink-0" />
                        <span>
                            Your trial ends in {timeText}. You&apos;re all set —
            ...
            <div className="flex items-center gap-3">
                <Progress
                    ...
                    className={`h-1.5 flex-1 ${
                        isUrgent
                            ? "[&>div]:bg-destructive-foreground"
                            : "[&>div]:bg-blue-500"
                    }`}
                />
                <span
                    className={`text-xs whitespace-nowrap ${isUrgent ? "text-destructive-foreground/80" : "text-blue-600"}`}
                >
                    Day {daysElapsed} of {totalTrialDays}
                </span>
            </div>
