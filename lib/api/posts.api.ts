...
export interface CreatePostData {
  ...
  videos?: string[];
  links?: PostLink[];
  /** 'publish' to post immediately, 'schedule' to defer publication */
  publishMode?: 'publish' | 'schedule';
  /** ISO 8601 date string. Required when publishMode is 'schedule'. */
  scheduledAt?: string;
}
...
export const postsApi = {
  ...
  // Create post (supports scheduling via publishMode + scheduledAt)
  ...
  /** Get all scheduled posts for a community */
  getScheduled: async (communityId: string): Promise<ApiSuccessResponse<Post[]>> => {
    return apiClient.get<ApiSuccessResponse<Post[]>>(`/posts/scheduled/${communityId}`);
  },

  /** Change the scheduled time of a post */
  reschedule: async (postId: string, scheduledAt: string): Promise<ApiSuccessResponse<Post>> => {
    return apiClient.patch<ApiSuccessResponse<Post>>(`/posts/${postId}/schedule`, { scheduledAt });
  },

  /** Cancel a scheduled post (moves it to draft) */
  cancelSchedule: async (postId: string): Promise<{ success: boolean; message: string }> => {
    return apiClient.delete<{ success: boolean; message: string }>(`/posts/${postId}/schedule`);
  },
};
